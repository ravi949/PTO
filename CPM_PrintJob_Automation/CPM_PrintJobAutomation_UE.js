    	...
    	try{
    		...
    		if (sc.type == sc.UserEventType.CREATE || sc.type == sc.UserEventType.EDIT || sc.type == sc.UserEventType.COPY){
    			...
    			if(automationStatus != '2' && automationStatus != '3'){
    				...
    				if(format != '' && pageCount != '' ){
    					redirect.toSuitelet({
        					scriptId : runtime.getCurrentScript().getParameter({name:'custscript_cpm_pjasuitelet1'}),
        					deploymentId : runtime.getCurrentScript().getParameter({name:'custscript_cpm_pjadeployment1'}),
        					parameters : {
        						'pjid' : printJob.id
        					}
        				});
